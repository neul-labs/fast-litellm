...
def benchmark_python_token_counting(test_data: List[Tuple[str, str]], iterations: int) -> BenchmarkResult:
...
def benchmark_shimmed_token_counting(test_data: List[Tuple[str, str]], iterations: int) -> BenchmarkResult:
    ...
    print("🚀 Running shimmed token counting benchmark (with monkeypatching overhead)...")
...
def benchmark_rust_token_counting(test_data: List[Tuple[str, str]], iterations: int) -> BenchmarkResult:
...
def compare_results(python_result: BenchmarkResult, rust_result: BenchmarkResult) -> Dict[str, Any]:
    """Compare two benchmark results and return comparison metrics"""
    speedup = python_result.avg_time / rust_result.avg_time
    
    # Calculate performance improvement percentage
    improvement = ((python_result.avg_time - rust_result.avg_time) / python_result.avg_time) * 100
    ...
    return {
        ...
        "throughput_improvement": ((rust_throughput - python_throughput) / python_throughput) * 100
    }
...
def main():
    parser = argparse.ArgumentParser(description="Benchmark token counting with Fast LiteLLM")
    parser.add_argument("--iterations", type=int, default=100, help="Number of benchmark iterations (default: 100)")
    parser.add_argument("--batch-size", type=int, default=10, help="Number of text samples per iteration (default: 10)")
    parser.add_argument("--output", type=str, help="Output file for JSON results")
    parser.add_argument("--mode", choices=["direct", "shimmed", "compare"],
                       default="compare",
    ...
    if HAS_RUST:
        if args.mode == "shimmed" or args.mode == "compare":
            ...
            shimmed_result = benchmark_shimmed_token_counting(test_data, args.iterations)
        ...
        if args.mode == "compare" and HAS_RUST:
            ...
            if hasattr(rust_result, 'avg_time') and hasattr(shimmed_result, 'avg_time'):
                ...
                overhead_percentage = (overhead_time / rust_result.avg_time) * 100 if rust_result.avg_time > 0 else 0
                print(f"Shimmed avg: {shimmed_result.avg_time:.6f}s")
                print(f"Direct Rust avg: {rust_result.avg_time:.6f}s")
                print(f"Shim overhead: {overhead_time:.6f}s ({overhead_percentage:.1f}%)")
            ...
            full_results = {
                ...
                "fast_litellm_info": {
                    "version": getattr(__import__('fast_litellm', fromlist=['__version__']), '__version__', 'unknown'),
