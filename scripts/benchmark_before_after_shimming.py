...
def run_benchmark_in_subprocess(use_fast_litellm: bool, test_data: List[Tuple[str, str]], iterations: int) -> BenchmarkRun:
...
"""
    ...
    try:
        ...
        return BenchmarkRun(
            ...
            metadata={
                "use_fast_litellm": use_fast_litellm,
                "success": True
            }
        )
...
def run_direct_comparison_benchmark(test_data: List[Tuple[str, str]], iterations: int) -> Tuple[BenchmarkRun, BenchmarkRun]:
    ...
    baseline_run = run_benchmark_in_subprocess(use_fast_litellm=False, test_data=test_data, iterations=iterations)
    ...
    shimmed_run = run_benchmark_in_subprocess(use_fast_litellm=True, test_data=test_data, iterations=iterations)
    print(f"   Fast LiteLLM complete: {shimmed_run.config.iterations} iterations")
    
    return baseline_run, shimmed_run


def calculate_comparison_metrics(baseline_run: BenchmarkRun, shimmed_run: BenchmarkRun) -> Dict[str, Any]:
...
def print_detailed_comparison(baseline_run: BenchmarkRun, shimmed_run: BenchmarkRun, metrics: Dict[str, Any]):
    ...
    # Average time
    print(f"{'Avg Time per Batch':<30} {format_time(metrics['baseline_avg_time']):<15} {format_time(metrics['shimmed_avg_time']):<15} {format_percentage(metrics['improvement_percent']):<15}")
    
    # Min time
    print(f"{'Min Time per Batch':<30} {format_time(metrics['baseline_min_time']):<15} {format_time(metrics['shimmed_min_time']):<15} {'N/A':<15}")
    
    # Max time
    print(f"{'Max Time per Batch':<30} {format_time(metrics['baseline_max_time']):<15} {format_time(metrics['shimmed_max_time']):<15} {'N/A':<15}")
    
    # Throughput
    print(f"{'Throughput (ops/s)':<30} {format_throughput(metrics['baseline_throughput']):<15} {format_throughput(metrics['shimmed_throughput']):<15} {format_percentage(metrics['throughput_improvement']):<15}")
    
    # Speedup
    print(f"{'Speedup':<30} {'N/A':<15} {format_speedup(metrics['speedup']):<15} {'vs Baseline':<15}")
    ...
    print(f"Shimmed performance is {metrics['speedup']:.2f}x {'faster' if metrics['speedup'] >= 1 else 'slower'} than baseline")
    
    print("\n" + "="*80)


def main():
    parser = argparse.ArgumentParser(description="Compare LiteLLM performance before and after Fast LiteLLM shimming")
    parser.add_argument("--iterations", type=int, default=30, help="Number of benchmark iterations per version (default: 30)")
    parser.add_argument("--batch-size", type=int, default=10, help="Number of operations per iteration (default: 10)")
    ...
    baseline_run, shimmed_run = run_direct_comparison_benchmark(test_data, args.iterations)
